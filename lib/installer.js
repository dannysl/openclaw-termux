...
export function getInstallStatus(options = {}) {
  // `skipProotProbe` avoids the `proot-distro login` fallback below, which can
  // block for up to 30s. Callers that only need the proot/ubuntu flags (for
  // example the setup steps that run before openclaw is installed) should pass
  // it, otherwise each status query adds up to 30s of dead time.
  const { skipProotProbe = false } = options;
  const PROOT_ROOTFS = '/data/data/com.termux/files/usr/var/lib/proot-distro/installed-rootfs';
  ...
  if (hasUbuntu) {
    ...
    if (!hasOpenClawInProot && !skipProotProbe) {
      try {
...
export function setupProotUbuntu() {
  ...
  // Remove any partial global install first: an interrupted `npm install -g`
  // leaves /usr/local/lib/node_modules/openclaw plus .openclaw-* staging dirs
  // behind, and npm then fails with `ENOTEMPTY: rename ... .openclaw-XXXX`
  // instead of recovering (#133).
  const setupScript = `
    ...
    rm -rf /usr/local/lib/node_modules/openclaw /usr/local/lib/node_modules/.openclaw-* 2>/dev/null || true
    npm install -g openclaw --omit=dev --no-audit --no-fund || {
      echo "Install failed, cleaning npm cache and retrying with build tools...";
      npm cache clean --force || true;
      rm -rf /usr/local/lib/node_modules/openclaw /usr/local/lib/node_modules/.openclaw-* || true;
...
/**
 * Read the gateway port from openclaw.json inside the proot rootfs.
 *
 * Upstream precedence is `--port` > `OPENCLAW_GATEWAY_PORT` > `gateway.port`
 * > 18789, so the dashboard URL must not assume 18789 (#124).
 */
export function getGatewayPort() {
  const DEFAULT_PORT = 18789;
  try {
    const configPath = path.join(PROOT_UBUNTU_ROOT, '.openclaw', 'openclaw.json');
    if (!fs.existsSync(configPath)) return DEFAULT_PORT;
    const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
    const port = config?.gateway?.port;
    if (Number.isInteger(port) && port > 0 && port <= 65535) return port;
  } catch { /* malformed or unreadable config */ }
  return DEFAULT_PORT;
}
