...
// Single source of truth: package.json. A hard-coded constant here silently
// drifted from the published version (#99).
const VERSION = (() => {
  try {
...
async function runSetup() {
  ...
  let status = getInstallStatus({ skipProotProbe: true });
  ...
  status = getInstallStatus({ skipProotProbe: true });
  if (!status.ubuntu) {
  ...
  console.log('');
  console.log(`Dashboard: http://127.0.0.1:${getGatewayPort()}`);
  console.log('═══════════════════════════════════════════');
}
...
function startGateway() {
  ...
  let i = 0;
  let started = false;
  // Respect a custom gateway.port instead of assuming 18789 (#124).
